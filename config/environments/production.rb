...
ActionController::Base.asset_host = Proc.new {|source, request|
  if request.ssl?
    "#{request.protocol}a#{source.hash % 10}.#{ActionMailer::Base.default_url_options[:host] || "luleka.com"}"
  else
    "#{request.protocol}a#{source.hash % 10}.#{ActionMailer::Base.default_url_options[:host] || "luleka.com"}"
  end
}
...
config.action_controller.session[:domain]  = '.luleka.com'

#--- action mailer
require 'smtp_tls'
config.action_mailer.raise_delivery_errors = false
config.action_mailer.default_url_options = {:host => 'luleka.com'}
config.action_mailer.delivery_method = :smtp
config.action_mailer.smtp_settings = {
  :address => "127.0.0.1",
  :port => 25,
  :domain => "www.luleka.net"
}
...
config.after_initialize do 
  ...
  Notifier.support_email  = '"Error | luleka.com" <error@luleka.com>'
