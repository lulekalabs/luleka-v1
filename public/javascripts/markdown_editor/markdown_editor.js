/* markdown editor */
if (window.Markdown == undefined) window.Markdown = {};
Markdown.Textarea = Class.create({
	initialize: function(textarea_id, preview_id) {
		this.enhance();
		...
		if (preview_id) {
		    ...
		    $(preview_id).update(converter.makeHtml(value));
				prettyPrint();
			}  
		...
		*/
		...
		var text_emphasize = function() {
			textarea.smartWrap('_', '_'); return false;
		}
		
		var text_bold = function() {
	...
	enhance: function() {
		// extending livepipe TextArea.Toolbar
		Control.TextArea.ToolBar.addMethods({
...
Markdown.Textarea.observe = function() {
	$$('textarea.markdown_editor').each(function(textarea) {
		if (textarea.previous() == null || textarea.previous().id !== 'markdown_toolbar') {
			new Markdown.Textarea(textarea);
		}
	});
}
