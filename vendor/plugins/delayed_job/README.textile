...
Job objects are serialized to yaml so that they can later be resurrected by the job runner. 
  ...
  or schedule a recurring job, like:

  Delayed::Job.schedule NewsletterJob.new('lorem ipsum...', Customers.find(:all).collect(&:email)), prio, start, 1.month

There is also a second way to get jobs in the queue: send_later. 
  
  BatchImporter.new(Shop.find(1)).send_later(:import_massive_csv, massive_csv)                                                    

There is also a way to schedule recurring jobs: schedule_later. 
  
  BatchImporter.new(Shop.find(1)).schedule_later(:import_massive_csv, time, period, priority, massive_csv)
