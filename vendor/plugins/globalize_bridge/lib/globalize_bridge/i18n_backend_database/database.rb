  ...
  module Backend
    class Database
      ...
      def translate(locale, key, options = {})
        ...
        unless entry || omit_key?(key) || @locale.default_locale?
        ...
        if !entry && !omit_key?(key) && (key.is_a?(String) || key.is_a?(Symbol))
        ...
        # write to cache unless we've already had a successful cache hit or the lookup entry is nil
        @cache_store.write(Translation.ck(@locale, key), entry) unless cache_lookup == true || entry.nil?
      ...
      def lookup(locale, key, scope=[], options={})
        ...
        if omit_key?(key)
          value = nil
        elsif @cache_store.exist?(cache_key)
      ...
      # define keys that should be ignored, e.g. activerecord.attributes...
      def omit_key?(key)
        case I18n.unescape_translation_key_without_scope(key.to_s)
          when /^activerecord\./ then true
