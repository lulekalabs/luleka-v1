          ...
          def translates_internal(facets, options)
            ...
            class_eval <<-STR
              ...
              class << self
                ...
                #
                def translated_attribute_name(facet, locale_code=nil)
                  if I18n.locale_language(locale_code || I18n.locale) == I18n.locale_language(I18n.default_locale)
                    facet.to_s
                  else
                    "\#{facet}_\#{I18n.locale_language(locale_code)}"
                  end
                end
                ...
                # Is facet translated?
                # Returns true if translated in given or current locale
                def translateable?(facet)
                  translated_attribute_names.include?(facet.to_sym)
                end
              ...
              # Returns true if translated in given or current locale
              def translateable?(facet)
                self.class.translateable?(facet)
              end
              
              # is the content of this facet translated or still empty
              def translated?(facet, locale_code=nil)
                if self.translateable?(facet) && I18n.base_locale
                  column_name = self.class.translated_attribute_name(facet, locale_code)
                  return respond_to?(column_name) && !!send(column_name)
                end
                false
              end
