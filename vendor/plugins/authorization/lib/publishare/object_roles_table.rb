    ...
    module UserExtensions
      ...
      module InstanceMethods
        ...
        def has_role( role_name, authorizable_obj = nil )
          ...
          role.clone_rights_from( role_name )
        end
        ...
        def has_right?( right_name, authorizable_obj = nil)
          if authorizable_obj
            rights = self.roles.find(:all, :conditions => ["authorizable_id = ? AND authorizable_type = ?", authorizable_obj.id, authorizable_obj.class.name] )
          else
            rights = self.roles.find(:all)
          end
          if rights.size>0
            rights.each do |right|
              return true if right.has_right?( right_name )
            end
          end
          false
        end
    ...
    module ModelExtensions
      ...
      module ClassMethods
        def acts_as_authorizable
          ...
          def accepts_right?( right_name, user )
            return false if user.nil?
            user.has_right? right_name, self
          end
