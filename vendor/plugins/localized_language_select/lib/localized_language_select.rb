...
module LocalizedLanguageSelect
  class << self
    ...
    def localized_languages_array
      I18n.translate(:languages).map {|key, value| [value, key.to_s.downcase]}.sort_by {|language| language.first.parameterize}
    end

    # Return array with codes and localized language names for array of language codes passed as argument
    # == Example
    #   priority_languages_array([:de, :es])
    #   # => [ ['German', 'de'], ['Español', 'es'] ]
    def priority_languages_array(language_codes=[])
      languages = I18n.translate(:languages)
      language_codes.blank? ? [] : language_codes.map {|code| [languages[code.to_s.downcase.to_sym], code.to_s.downcase]}
    end
  end
end
  ...
  module Helpers
    ...
    module FormOptionsHelper
      ...
      def localized_language_select_tag(name, selected_value = nil, priority_languages = nil, html_options = {})
        select_tag name.to_sym, localized_language_options_for_select(selected_value, priority_languages), html_options.stringify_keys
      end

      # Returns a string of option tags for languages according to locale. Supply the language code in lower-case ('en', 'es') 
      ...
      def localized_language_options_for_select(selected = nil, priority_languages = nil)
        ...
        return language_options + options_for_select(LocalizedLanguageSelect::localized_languages_array - 
          LocalizedLanguageSelect::priority_languages_array(priority_languages), selected)
      end
