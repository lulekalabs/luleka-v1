    ...
    module Flag
      ...
      module SingletonMethods
        ...
        def find_flags_by_user(user) 
          ...
          Flag.find(:all,
            :conditions => ["user_id = ? and flaggable_type = ?", user.id, flaggable.class.name],
            :order => "created_at DESC"
          )
        end
      ...
      module InstanceMethods
        ...
        def user_has_flagged?(user, reason = nil)
          conditions = reason.nil? ? {} : {:reason => reason}
        ...
        # flag set
        def count_flags_with_reason(reason)
          flags.count(:conditions => ["flags.reason = ?", reason])
        end
        ...
        def add_flag(options)
          ...
          # flagged
          flagged(flag.reason, count_flags_with_reason(flag.reason))
