# taken from http://www.slashdotdash.net/2007/12/03/rails-2-upgrade-notes/
...
namespace 'views' do
  ...
  task :rename do
    ...
    Dir.glob('app/views/**/*.rjs').each do |file|
      puts `svn mv #{file} #{file.gsub(/\.rjs$/, '.js.rjs')}` unless file.index('js.')
    end
    
    Dir.glob('app/views/**/[^_]*.haml').each do |file|
      puts `svn mv #{file} #{file.gsub(/\.haml$/, '.html.haml')}` unless file.index('html.')
    end
